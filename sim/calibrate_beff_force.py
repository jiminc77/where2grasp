...
def _settle(scene, rods, chunk=200, max_steps=16000, drift_tol=5e-3, interval=0.01, consec=3):
    ...
    window = np.full((consec, len(rods), n_envs), np.inf)   # rolling last-`consec` per-cell drifts
    steps = 0
    while steps < max_steps:
        ...
        d = np.stack([np.abs(c - p) for c, p in zip(cur, prev)])   # (n_rods, n_envs)
        window = np.concatenate([window[1:], d[None]], axis=0)      # roll in the newest chunk
        quiet_run = np.where(d < thr, quiet_run + 1, 0)
        prev = cur
        if (quiet_run >= consec).all():
            break
    converged = (window < thr).all(axis=0)     # last `consec` chunks ALL quiet (consecutive), not just final
    return bool((quiet_run >= consec).all()), steps, converged, window
...
def force_calibrate_baseline(raw_es, b_eff_sizing, interval, m0=ic.BASELINE_ARM_MASS,
    ...
    conv_sw, steps_sw, csw, wsw = _settle(scene, rods, max_steps=max_steps, interval=interval)
    ...
    conv_tot, steps_tot, ctot, wtot = _settle(scene, rods, max_steps=max_steps, interval=interval)
    z_tot = [vertices(r).copy() for r in rods]
    settle_window = np.maximum(wsw, wtot)            # (consec, n_lengths, n_materials): worst per-chunk drift
    settle_conv = (settle_window < 5e-3 * interval).all(axis=0)   # both phases' last-`consec` chunks quiet
    ...
    return dict(interval=interval, m0=m0, ell=ell.tolist(),
                ...
                settle_window=settle_window.tolist(), settle_converged=settle_conv.tolist(),
