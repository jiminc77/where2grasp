...
def main():
    ...
    np.savez(MAN / "independent_closure_results.npz",
             ...
             **{f"sweep_drift_{iv}": np.asarray(sweeps[iv]["per_ell_env_drift"]) for iv in REALIZED},
             **{f"calib_drift_{iv}": np.asarray(calib[iv]["settle_drift"]) for iv in REALIZED},
             **{f"calib_F2_drift_{iv}": np.asarray(calib_F2[iv]["settle_drift"]) for iv in REALIZED},
             prospective_drift=np.asarray(prosp_sweep["per_ell_env_drift"]),
