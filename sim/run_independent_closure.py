...
def main():
    ...
    np.savez(MAN / "independent_closure_results.npz",
             ...
             **{f"sweep_window_{iv}": np.asarray(sweeps[iv]["per_ell_env_window"]) for iv in REALIZED},
             **{f"calib_window_{iv}": np.asarray(calib[iv]["settle_window"]) for iv in REALIZED},
             **{f"calib_F2_window_{iv}": np.asarray(calib_F2[iv]["settle_window"]) for iv in REALIZED},
             prospective_window=np.asarray(prosp_sweep["per_ell_env_window"]),
             drift_thr_002=float(5e-3 * 0.02), drift_thr_001=float(5e-3 * 0.01),
